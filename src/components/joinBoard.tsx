...
const JoinBoard = () => {
 ...
 const reduxStep = useSelector((state: RootState) => state.gameServerBoard.step);
 const [step, setStep] = useState<boolean>(reduxStep);
 const [color, setColor] = React.useState<boolean>(false);
 const reduxBoard = useSelector((state: RootState) => state.gameServerBoard.board);
 const [board, setBoard] = useState<Board>(reduxBoard);
 const [serverId, setServerId] = useState<string>(id!);
 console.log(serverId, "ID");
 const dispatch = useDispatch<RootDispatxh>();

 React.useEffect(() => {
  if (data?.board) {
   console.log("Данные загружены:", data.board);
   dispatch(updateBoard(data.board));
   setBoard(data.board);
   setStep(data.step);
  } else {
   console.warn("Ждём загрузку board, текущее значение:", data);
  }
 }, [data, dispatch]);
 React.useEffect(() => {
